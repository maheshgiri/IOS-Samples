...
extension PlaySoundViewController: AVAudioPlayerDelegate {
    ...
    func playSound(rate: Float? = nil, pitch: Float? = nil, echo: Bool = false, reverb: Bool = false) {
        ...
        if echo == true && reverb == true {
            connectAudioNodes(nodes: audioPlayNode, changeRatePitchNode, echoNode, reverbNode, audioEngine.outputNode)
        } else if echo == true {
            connectAudioNodes(nodes: audioPlayNode, changeRatePitchNode, echoNode, audioEngine.outputNode)
        } else if reverb == true {
            connectAudioNodes(nodes: audioPlayNode, changeRatePitchNode, reverbNode, audioEngine.outputNode)
        } else {
            connectAudioNodes(nodes: audioPlayNode, changeRatePitchNode, audioEngine.outputNode)
        }
        ...
        audioPlayNode.scheduleFile(audioFile, at: nil) {
            ...
            if let lastRenderTime = self.audioPlayNode.lastRenderTime, let playerTime = self.audioPlayNode.playerTime(forNodeTime: lastRenderTime) {
    ...
    func stopAudio() {
        ...
        if let audioPlayerNode = audioPlayNode {
